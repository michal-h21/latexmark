...
latexmark.init = function(extensions)
   local extensions = #extensions > 0 and extensions or { smart = true, definition_lists=true}
   latexmark.parser = lunamark.reader.markdown.new(writer, extensions)
end

latexmark.template = function(tpl)
  if string.len(tpl) == 0 then return false,"No template" end
  local f = io.open(kpse.find_file(tpl),"r")
  if not f then 
    print("latexmark: cannot load template file: "..tpl)
  else
    writer.template = f:read("*all")
    f:close()
  end
end
...
latexmark.process = function()
  ...
  local template = writer.template or "$body"
